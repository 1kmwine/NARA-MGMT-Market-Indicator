...
환율·소비지출전망CSI·가처분소득·주류소비 4개 지표를 보여주는 대시보드예요.
...
- **http://localhost:8000/docs** → API 테스트 화면 (여기서 `/api/fx`, `/api/csi`, `/api/income`, `/api/alcohol` 각각을 눌러서 데이터가 잘 나오는지 확인 가능)
...
```
backend/    FastAPI 서버 — ECOS/KOSIS 호출, 캐싱, /api/* 라우트
